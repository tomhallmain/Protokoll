...
def _cleanup_old_logs(log_dir: Path, logger: logging.Logger) -> None:
    ...
    """
    try:
        log_files: List[Path] = list(log_dir.glob('protokoll_*.log'))
        if len(log_files) <= 10:
            return
        ...
        for log_file in log_files:
            try:
                # Extract date from filename (format: protokoll_YYYY-MM-DD.log)
...
def get_logger(module_name: str) -> logging.Logger:
    ...
    logger: logging.Logger = logging.getLogger(f"protokoll.{module_name}")
    ...
    log_dir: Path = Path(appdata_dir) / 'protokoll' / 'logs'
    ...
    log_file: Path = log_dir / f'protokoll_{date_str}.log'
